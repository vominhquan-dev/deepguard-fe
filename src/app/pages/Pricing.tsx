...
import {
  Shield, ArrowRight, Sun, Moon, CheckCircle2, X,
  Zap, Star, Building2, HelpCircle, ChevronDown, ChevronUp,
  Lock, Trash2, Eye, Github,
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0F172A]" style={{ fontFamily: "'Inter', sans-serif" }}>
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <span className="text-slate-900 dark:text-white" style={{ fontWeight: 700, fontSize: '18px', letterSpacing: '-0.4px' }}>
              ...
              <span className="text-slate-400 dark:text-slate-500" style={{ fontWeight: 400, fontSize: '14px' }}>AI</span>
            </span>
          </div>
      ...
      <section className="pt-32 pb-16 text-center relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <span className="inline-block px-3 py-1 rounded-full bg-[#2563EB]/10 text-[#2563EB] dark:text-[#22D3EE] mb-5" style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
              Pricing
            </span>
            <h1 className="text-slate-900 dark:text-white mb-4" style={{ fontSize: 'clamp(32px, 5vw, 52px)', fontWeight: 800, letterSpacing: '-1.5px', lineHeight: 1.1 }}>
              Simple, Transparent Pricing
            </h1>
            <p className="text-slate-500 dark:text-slate-400 max-w-xl mx-auto mb-8" style={{ fontSize: '18px', lineHeight: 1.7 }}>
            ...
            {plans.map(({ name, tagline, price, icon: Icon, iconColor, iconBg, border, highlight, badge, features, cta, ctaVariant }, i) => {
              return (
                ...
                >
                  ...
                  <h3 className="text-slate-900 dark:text-white mb-1" style={{ fontSize: '20px', fontWeight: 800 }}>{name}</h3>
                  <p className="text-slate-500 dark:text-slate-400 mb-6" style={{ fontSize: '13px' }}>{tagline}</p>
      ...
      <section className="border-y border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0C1220] py-12">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-center text-slate-400 dark:text-slate-500 mb-8" style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
            Security & Compliance
          </p>
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { icon: Lock, title: 'SOC 2 Type II', desc: 'Certified infrastructure' },
              { icon: Eye, title: 'GDPR Compliant', desc: 'Privacy-first architecture' },
              { icon: Trash2, title: 'Zero Data Retention', desc: 'Files deleted in 60 seconds' },
              { icon: Shield, title: 'TLS 1.3 Encryption', desc: 'All transfers secured' },
            ].map(({ icon: Icon, title, desc }) => (
              <div key={title} className="flex items-center gap-3 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                <div className="w-9 h-9 rounded-lg bg-emerald-500/10 flex items-center justify-center flex-shrink-0">
                  <Icon className="w-4 h-4 text-emerald-500" />
                </div>
                <div>
                  <p className="text-slate-900 dark:text-white" style={{ fontSize: '13px', fontWeight: 700 }}>{title}</p>
      ...
      <section className="py-24 bg-slate-50 dark:bg-[#0F172A]">
        <div className="max-w-3xl mx-auto px-6">
          ...
          >
            ...
            <h2 className="text-slate-900 dark:text-white mb-3" style={{ fontSize: '32px', fontWeight: 800, letterSpacing: '-0.8px' }}>
              Frequently Asked Questions
            </h2>
            <p className="text-slate-500 dark:text-slate-400" style={{ fontSize: '16px' }}>
              Can't find what you're looking for?{' '}
              <button onClick={() => navigate('/contact')} className="text-[#2563EB] dark:text-[#22D3EE] hover:underline" style={{ fontWeight: 600 }}>
                Contact our team
              </button>
            </p>
          </motion.div>
          ...
          <div className="space-y-3">
            {faqs.map(({ q, a }, i) => (
              ...
              >
                ...
                >
                  <span className="text-slate-900 dark:text-slate-200 pr-4" style={{ fontSize: '14px', fontWeight: 600 }}>{q}</span>
                  {openFaq === i ? (
                ...
                {openFaq === i && (
                  ...
                  >
                    <p className="text-slate-500 dark:text-slate-400 pt-4" style={{ fontSize: '14px', lineHeight: 1.75 }}>{a}</p>
                  </motion.div>
                )}
              </motion.div>
            ))}
      ...
      <section className="py-20 bg-[#2563EB] relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-white mb-4" style={{ fontSize: '32px', fontWeight: 800, letterSpacing: '-0.8px' }}>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {[
              ...
              <button key={label} onClick={action} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors" style={{ fontSize: '14px', fontWeight: 500 }}>
                {label}
              </button>
            ))}
