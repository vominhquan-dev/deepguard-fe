...
import { Shield, Sun, Moon, ArrowLeft, Github, Lock, Eye, Trash2, Server, FileText, Bell } from 'lucide-react';
      ...
      <div className="max-w-5xl mx-auto px-6 pt-32 pb-24">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[#2563EB]/30 bg-[#2563EB]/10 mb-6">
            ...
            <span className="text-[#22D3EE]" style={{ fontSize: '12px', fontWeight: 600 }}>
              Last updated: March 1, 2026
            </span>
          </div>
          <h1 className="text-slate-900 dark:text-white mb-4" style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, letterSpacing: '-1px', lineHeight: 1.1 }}>
            Privacy Policy
          </h1>
          <p className="text-slate-500 dark:text-slate-400 max-w-2xl" style={{ fontSize: '16px', lineHeight: 1.7 }}>
            We built DeepGuard AI with privacy as a core principle, not an afterthought. Here's exactly how we handle your data.
          </p>
          ...
          <div className="grid sm:grid-cols-3 gap-4 mt-8">
            {[
              ...
              { icon: Lock, label: 'SOC 2 Type II certified', color: '#8B5CF6' },
            ].map(({ icon: Icon, label, color }) => (
              <div key={label} className="flex items-center gap-3 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1E293B]">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${color}18` }}>
                  <Icon className="w-4 h-4" style={{ color }} />
                </div>
                <span className="text-slate-700 dark:text-slate-300" style={{ fontSize: '13px', fontWeight: 600 }}>{label}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 rounded-2xl bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700 p-4">
              <p className="text-slate-500 dark:text-slate-400 mb-3" style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Contents</p>
          ...
          <div className="lg:col-span-3 space-y-8">
            {sections.map(({ id, title, icon: Icon, content }, i) => (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-slate-900 dark:text-white" style={{ fontSize: '16px', fontWeight: 700 }}>{title}</h2>
                </div>
                <div className="text-slate-600 dark:text-slate-400 space-y-3" style={{ fontSize: '14px', lineHeight: 1.75 }}>
                  {content.split('\n\n').map((para, j) => (
                    <p key={j}>{para}</p>
                  ))}
                </div>
            ...
            <div className="rounded-2xl bg-gradient-to-br from-[#2563EB] to-blue-800 p-6 text-white">
              <h2 className="text-white mb-2" style={{ fontSize: '16px', fontWeight: 700 }}>Privacy Questions?</h2>
              <p className="text-blue-100 mb-4" style={{ fontSize: '14px', lineHeight: 1.7 }}>
                If you have questions about this Privacy Policy or want to exercise your data rights, please contact our Data Protection Officer.
              </p>
              <div className="space-y-2">
                <a href="mailto:privacy@deepguard.ai" className="flex items-center gap-2 text-blue-100 hover:text-white transition-colors" style={{ fontSize: '14px' }}>
                  📧 privacy@deepguard.ai
                </a>
                <a href="#" className="flex items-center gap-2 text-blue-100 hover:text-white transition-colors" style={{ fontSize: '14px' }}>
