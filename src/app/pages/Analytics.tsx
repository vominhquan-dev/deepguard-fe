  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#1E293B] border border-slate-700 rounded-xl p-3 shadow-xl">
        <p className="text-slate-300 mb-2" style={{ fontSize: '12px', fontWeight: 600 }}>{label}</p>
        {payload.map((entry: any) => (
          <p key={entry.name} style={{ color: entry.color, fontSize: '13px', fontWeight: 700 }}>
    ...
    <DashboardLayout>
      <div className="p-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {statsCards.map(({ label, value, sub, trend, icon: Icon, color, bg }, i) => (
            <motion.div
              key={label}
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                {trend === 'up' ? (
                  <div className="flex items-center gap-1 text-emerald-500">
                    ...
                    <span style={{ fontSize: '11px', fontWeight: 700 }}>+12%</span>
                  </div>
                ) : trend === 'down' ? (
              ...
              <p className="text-slate-400 dark:text-slate-600 mt-1" style={{ fontSize: '11px' }}>{sub}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-5 mb-5">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-slate-900 dark:text-white" style={{ fontSize: '16px', fontWeight: 700 }}>Weekly Scan Activity</h2>
                <p className="text-slate-400 dark:text-slate-500" style={{ fontSize: '13px' }}>Total scans vs deepfakes — last 7 days</p>
              </div>
              <div className="flex gap-4">
                {[{ label: 'Total', color: '#2563EB' }, { label: 'Deepfakes', color: '#EF4444' }, { label: 'Authentic', color: '#10B981' }].map(({ label, color }) => (
                  <div key={label} className="flex items-center gap-1.5">
                    <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: color }} />
                    <span className="text-slate-500 dark:text-slate-400" style={{ fontSize: '11px', fontWeight: 600 }}>{label}</span>
                  </div>
                ))}
              </div>
            </div>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={weeklyData} barGap={4} barCategoryGap="30%">
                ...
                <Bar dataKey="scans" name="Total" fill="#2563EB" radius={[4, 4, 0, 0]} fillOpacity={0.8} />
                <Bar dataKey="deepfakes" name="Deepfakes" fill="#EF4444" radius={[4, 4, 0, 0]} fillOpacity={0.8} />
                <Bar dataKey="authentic" name="Authentic" fill="#10B981" radius={[4, 4, 0, 0]} fillOpacity={0.8} />
              </BarChart>
            </ResponsiveContainer>
          </motion.div>
          ...
          >
            <h2 className="text-slate-900 dark:text-white mb-1" style={{ fontSize: '16px', fontWeight: 700 }}>Scan Types</h2>
            <p className="text-slate-400 dark:text-slate-500 mb-4" style={{ fontSize: '13px' }}>Distribution by media type</p>
                ...
                <div key={type} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-0.5">
                      <span className="text-slate-700 dark:text-slate-300" style={{ fontSize: '12px', fontWeight: 600 }}>{type}</span>
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <motion.div
            ...
            className="p-6 rounded-2xl bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700"
          >
            <h2 className="text-slate-900 dark:text-white mb-1" style={{ fontSize: '16px', fontWeight: 700 }}>Deepfake Detection Rate</h2>
            <p className="text-slate-400 dark:text-slate-500 mb-5" style={{ fontSize: '13px' }}>Monthly % of deepfakes detected — last 6 months</p>
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={trendData}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ background: '#1E293B', border: '1px solid #334155', borderRadius: '12px', fontSize: '12px', color: '#fff' }}
                />
                <Area type="monotone" dataKey="rate" stroke="#EF4444" strokeWidth={2.5} fill="url(#rateGradient)" dot={{ fill: '#EF4444', r: 4 }} activeDot={{ r: 6 }} />
              </AreaChart>
            </ResponsiveContainer>
          </motion.div>
          ...
          <motion.div
            ...
            className="p-6 rounded-2xl bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700"
          >
            <h2 className="text-slate-900 dark:text-white mb-1" style={{ fontSize: '16px', fontWeight: 700 }}>Risk Distribution</h2>
            <p className="text-slate-400 dark:text-slate-500 mb-4" style={{ fontSize: '13px' }}>Breakdown of verdicts across all scans</p>
                ...
                {riskDistribution.map(({ name, value, fill }) => (
                  <div key={name}>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-slate-600 dark:text-slate-400" style={{ fontSize: '12px', fontWeight: 600 }}>{name.split('\n')[0]}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{ width: `${value}%`, backgroundColor: fill }} />
                      </div>
