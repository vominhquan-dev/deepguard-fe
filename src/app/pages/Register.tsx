...
import {
  ...
  CheckCircle2, Sun, Moon, User, Star, BarChart3, FileText,
} from 'lucide-react';
import { useTheme } from '../contexts/ThemeContext';

const benefits = [
  { icon: BarChart3, title: 'Full scan history', desc: 'Track every detection with detailed logs and timestamps.' },
  { icon: FileText, title: 'Downloadable PDF reports', desc: 'Export litigation-ready reports for any scan result.' },
    ...
    <div className="min-h-screen flex" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="hidden lg:flex flex-col w-[460px] min-h-screen bg-[#060D1A] relative overflow-hidden p-12 flex-shrink-0">
        ...
        <div className="relative flex items-center gap-3 mb-12">
          ...
          <div>
            <span className="text-white" style={{ fontWeight: 700, fontSize: '17px', letterSpacing: '-0.3px' }}>
              Deep<span className="text-[#22D3EE]">Guard</span>
            </span>
            <span className="block text-slate-600" style={{ fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
              AI Platform
            </span>
          </div>
        </div>
        ...
        <div className="relative flex-1 flex flex-col justify-center">
          <div className="inline-flex items-center gap-2 mb-5">
            ...
            <span className="text-emerald-400" style={{ fontSize: '12px', fontWeight: 600 }}>50,000+ active users worldwide</span>
          </div>

          <h2 className="text-white mb-4" style={{ fontSize: '28px', fontWeight: 800, lineHeight: 1.2, letterSpacing: '-0.5px' }}>
            Join the most advanced deepfake detection platform
          </h2>
          <p className="text-slate-400 mb-10" style={{ fontSize: '14px', lineHeight: 1.7 }}>
            Create your free account and start protecting yourself from synthetic media threats in under 60 seconds.
          </p>
          ...
          <div className="space-y-5">
            {benefits.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-white mb-0.5" style={{ fontSize: '13px', fontWeight: 600 }}>{title}</p>
                  <p className="text-slate-500" style={{ fontSize: '12px', lineHeight: 1.5 }}>{desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="relative flex items-center gap-2 pt-8 border-t border-slate-800/80">
          ...
          <span className="text-slate-400" style={{ fontSize: '12px' }}>No credit card required for the Free plan</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-10 bg-slate-50 dark:bg-[#0F172A] overflow-y-auto">
        <div className="w-full max-w-[420px] py-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            {/* Header */}
            <div className="mb-6">
              <h1 className="text-slate-900 dark:text-white mb-2" style={{ fontSize: '26px', fontWeight: 800, letterSpacing: '-0.5px' }}>
                Create your account
              </h1>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {([
                ...
                >
                  <p className="text-slate-900 dark:text-white" style={{ fontSize: '13px', fontWeight: 700 }}>{label}</p>
                  <p className="text-slate-400" style={{ fontSize: '11px', marginTop: '2px' }}>{sub}</p>
                  {plan === id && (
                    <div className="mt-1.5 w-4 h-4 rounded-full flex items-center justify-center" style={{ backgroundColor: accent }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {form.confirm && form.confirm !== form.password && (
                  <p className="text-red-400 mt-1" style={{ fontSize: '11px' }}>Passwords do not match</p>
                )}
              </div>
              ...
              <label className="flex items-start gap-2.5 cursor-pointer">
                ...
                />
                <span className="text-slate-500 dark:text-slate-400" style={{ fontSize: '12px', lineHeight: 1.5 }}>
                  I agree to the{' '}
                  <button type="button" onClick={() => navigate('/privacy')} className="text-[#2563EB] dark:text-[#22D3EE] hover:underline" style={{ fontWeight: 600 }}>
                    Privacy Policy
                  </button>{' '}
            ...
            <div className="relative my-5">
              ...
              <div className="relative flex justify-center">
                <span className="px-3 bg-slate-50 dark:bg-[#0F172A] text-slate-400" style={{ fontSize: '12px' }}>or</span>
              </div>
            </div>
            <button
              onClick={() => { toast.info('Continuing as guest — 3 free scans available.'); navigate('/dashboard'); }}
