    ...
    <aside className="fixed left-0 top-0 h-screen w-60 flex flex-col bg-white dark:bg-[#0F172A] border-r border-slate-200 dark:border-slate-800 z-40">
      ...
      <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-200 dark:border-slate-800">
        ...
        <div>
          ...
          <span className="block text-slate-400 dark:text-slate-500" style={{ fontSize: '10px', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            AI Platform
          </span>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-5 space-y-1 overflow-y-auto">
        <p className="px-3 mb-3 text-slate-400 dark:text-slate-600" style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          Main Menu
        </p>
        {navItems.map(({ to, label, icon: Icon }) => (
          ...
          >
            {({ isActive }) => (
              <>
                <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-[#2563EB] dark:text-[#22D3EE]' : ''}`} />
                ...
                {!isActive && label === 'Realtime Monitor' && (
                  <span
                    ...
                    style={{ fontSize: '8px', fontWeight: 800, backgroundColor: 'rgba(239,68,68,0.15)', color: '#EF4444', letterSpacing: '0.06em' }}
                  >
                    LIVE
                  </span>
                )}
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#22D3EE]" />}
              </>
            )}
          </NavLink>
        </div>
      ...
      <div className="px-3 py-4 border-t border-slate-200 dark:border-slate-800 space-y-2">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="text-slate-600 dark:text-slate-300" style={{ fontSize: '13px', fontWeight: 500 }}>
              {theme === 'dark' ? 'Dark Mode' : 'Light Mode'}
            </span>
          </div>
          <div className={`w-8 h-4 rounded-full relative transition-colors duration-300 ${theme === 'dark' ? 'bg-[#2563EB]' : 'bg-slate-300'}`}>
            <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow transition-all duration-300 ${theme === 'dark' ? 'left-4' : 'left-0.5'}`} />
          </div>
        </button>
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-slate-900 dark:text-slate-200 truncate" style={{ fontSize: '12px', fontWeight: 600 }}>Admin User</p>
