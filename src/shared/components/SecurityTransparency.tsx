...
const certs = [
  ...
  { icon: Server, label: 'ISO 27001', sub: 'In Progress' },
  { icon: RefreshCw, label: 'Pen Tested', sub: 'Quarterly' },
];
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 mb-6">
            ...
            <span className="text-emerald-400" style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.06em' }}>
          ...
          <h2
            ...
            style={{ fontSize: 'clamp(28px, 4vw, 42px)', fontWeight: 800, letterSpacing: '-1px', lineHeight: 1.1 }}
          >
          ...
          >
            Every claim about how we handle your data is backed by auditable architecture,
            third-party certifications, and a legally binding privacy policy — not just marketing copy.
          </p>
        </motion.div>

        {/* Pillars grid */}
        <div className="grid md:grid-cols-2 gap-5 mb-12">
          {pillars.map(({ icon: Icon, badge, badgeBg, title, desc, accent }, i) => (
            <motion.div
              key={title}
            ...
            >
              ...
              <h3 className="text-white mb-3" style={{ fontSize: '17px', fontWeight: 700 }}>
                {title}
              </h3>
              <p className="text-slate-400" style={{ fontSize: '14px', lineHeight: 1.75 }}>
                {desc}
              </p>
        ...
        >
          {certs.map(({ icon: Icon, label, sub }) => (
            <div
              key={label}
              className="flex items-center gap-3 px-5 py-3 rounded-xl bg-slate-800/60 border border-slate-700/60"
            >
              <Icon className="w-4 h-4 text-[#22D3EE]" />
              <div>
                <p className="text-white" style={{ fontSize: '13px', fontWeight: 700 }}>{label}</p>
                <p className="text-slate-500" style={{ fontSize: '10px', fontWeight: 500 }}>{sub}</p>
              </div>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="text-center p-6 rounded-2xl border border-slate-700/50 bg-slate-800/30"
          style={{ backdropFilter: 'blur(8px)' }}
        >
          <p className="text-slate-400" style={{ fontSize: '14px', lineHeight: 1.7 }}>
            Our security practices are independently audited by{' '}
            <span className="text-white font-semibold">CrowdStrike Falcon</span> quarterly, and our privacy policy
            is reviewed by{' '}
            <span className="text-white font-semibold">EFF-certified attorneys</span>.
            {' '}Read the full{' '}
            <a
              href="#"
